...
class AcousticModel(object):
    ...
    def create_training_rnn(self,is_mpi, input_keep_prob, output_keep_prob, grad_clip, learning_rate, lr_decay_factor,
                            use_iterator=False,is_sync=0,is_chief=False):
        """
    ...
    def build_dataset(input_set, batch_size, max_input_seq_length, max_target_seq_length,
        ...
        if isinstance(input_set, str):
            ...
            def _parse(f):
                ...
                #audio = tf.cond(tf.greater(length,max), lambda: tf.slice(audio,[0,0],[max,y]) , lambda: audio)
    ...
    def run_train_step(self, sess, mini_batch_size, rnn_state_reset_ratio, run_options=None, run_metadata=None):
        ...
        if mini_batch_num > 0:
            ...
            # Step result
            client.update_task_info({'loss':float(mean_loss),'error_rate': float(mean_error_rate),'time': int(time.time() - start_time)})
